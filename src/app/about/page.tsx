...
export default async function AboutPage() {
  const [teamMembers, rawTimeline, testimonials, certifications, ps] = await Promise.all([
    getTeamMembers(),
    getTimeline(),
    getTestimonials(),
    getCertifications(),
  ...
  // Swiftrooms was founded in 2011, not 2009, and the Cortizo partnership
  // followed in 2012 — force-correct regardless of what the CMS timeline
  // document has stored for these entries.
  const TIMELINE_YEAR_OVERRIDES: Record<string, string> = {
    Founded: "2011",
    "Cortizo Partnership": "2012",
  };
  const timeline = rawTimeline.map((entry) =>
    TIMELINE_YEAR_OVERRIDES[entry.title] ? { ...entry, year: TIMELINE_YEAR_OVERRIDES[entry.title] } : entry
  );
