    ...
    <ScrollReveal key={product.id} delay={index * 0.08}>
      <div className="bg-white p-5 md:p-8 lg:p-10 hover:bg-[#f8f9fa] transition-colors duration-300">
        <div className="w-full h-36 md:h-48 mb-6 md:mb-8 relative overflow-hidden bg-[#f0fdf4]">
          {heroImage ? (
            <Image src={heroImage} alt={altTextFor(heroImage, product.name)} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
          ) : (
        ...
        <p className="text-lg md:text-xl font-semibold text-[#1c1c1e] mb-3">{product.name}</p>
        ...
        <ScrollReveal>
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-lg md:text-xl font-semibold text-[#1c1c1e]">Compare models.</p>
            </div>
            <button
...
export default function CategoryClient({
  ...
  const content = CATALOGUE_CONTENT[category.slug];
  const faqs = [...(category.faqs ?? []), ...(content?.extraFaqs ?? [])];
  ...
  return (
    <>
      <section className="pt-32 pb-12 md:pt-44 md:pb-20 lg:pt-52 lg:pb-28">
        <div className="max-w-screen-xl mx-auto px-5 md:px-8 lg:px-10">
          ...
          <ScrollReveal delay={0.1}>
            ...
            <h1 className="text-headline text-[#1c1c1e] mb-3 md:mb-4 max-w-3xl">{h1Override ?? category.name}</h1>
            <p className="text-base md:text-xl text-[#6b7280] italic mb-6 md:mb-8">{category.tagline}</p>
          </ScrollReveal>
          <ScrollReveal delay={0.2}>
            <p className="text-body-lg text-[#6b7280] max-w-2xl">{descriptionOverride ?? category.description}</p>
          </ScrollReveal>
        </div>
      </section>
      ...
      <ContentBlocks blocks={content?.afterHero} />
      ...
      <ContentBlocks blocks={content?.afterComparison} />

      <WorksWellWith categorySlug={category.slug} categories={allCategories} level={content?.worksWellWithLevel} />

      <ContentBlocks blocks={content?.afterWorksWellWith} />

      <FAQAccordion faqs={faqs} level={content?.faqLevel} heading={content?.faqHeading} />
              ...
              <ScrollReveal>
                <div className="flex items-end justify-between mb-8 md:mb-12">
                  <div>
                    ...
                    <p className="text-title text-[#1c1c1e]">Further reading.</p>
                  </div>
                  ...
                  <ScrollReveal key={post.slug} delay={i * 0.08}>
                    <Link href={`/technical/blog/${post.slug}`} className="group block bg-white hover:bg-[#f8f9fa] transition-colors h-full">
                      ...
                      <div className="p-5 md:p-6">
                        ...
                        <p className="font-semibold text-[#1c1c1e] text-sm leading-snug group-hover:text-[#007969] transition-colors mb-2">{post.title}</p>
      ...
      <ContentBlocks blocks={content?.beforeOtherRanges} />
      ...
      <ContentBlocks blocks={content?.afterOtherRanges} />
