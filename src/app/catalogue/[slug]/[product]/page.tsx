...
import { PRODUCT_CONTENT } from "@/lib/productContent";
...
const productMetaOverrides: Record<string, { title: string; description: string; keywords: string[] }> = {
  "aluminium-windows/aluminium-sliding-windows": {
    title: "Aluminium Sliding Windows",
    description:
      "Explore premium aluminium sliding windows by Swiftrooms, designed for style, durability and energy efficiency. Get your free quote today",
    keywords: ["aluminium sliding windows"],
  },
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const override = productMetaOverrides[`${slug}/${productSlug}`];
  const metaTitle = override?.title ?? `${product.name} UAE — ${product.brand} Glazing Specialist`;
  const metaDescription = override?.description ?? product.description;
  const ogTitle = override ? `${override.title} | Swiftrooms` : `${product.name} | ${product.brand} UAE | Swiftrooms`;
  return {
    title: metaTitle,
    description: metaDescription,
    ...(override ? { keywords: override.keywords } : {}),
    alternates: { canonical: `${SITE_URL}/catalogue/${slug}/${productSlug}` },
    openGraph: {
      type: "website",
      title: ogTitle,
      description: metaDescription,
      url: `${SITE_URL}/catalogue/${slug}/${productSlug}`,
      images: [
        product.image
          ? { url: product.image, alt: product.name }
          : { url: `${SITE_URL}/brand/og-default.jpg`, width: 1200, height: 630, alt: product.name },
      ],
    },
  };
}
...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const content = PRODUCT_CONTENT[`${slug}/${productSlug}`];
  ...
  const productSchema = {
    ...
    offers: {
      ...
      url: `${SITE_URL}/catalogue/${cat.slug}/${product.slug}`,
      seller: {
        "@type": "Organization",
        name: "Swiftrooms",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: cat.name, item: `${SITE_URL}/catalogue/${cat.slug}` },
      { "@type": "ListItem", position: 4, name: product.name, item: `${SITE_URL}/catalogue/${cat.slug}/${product.slug}` },
    ],
  };

  // Mirrors the FAQAccordion below, which renders the same content.faqs.
  ...
  return (
    <>
        ...
        <section className="max-w-screen-xl mx-auto px-5 md:px-8 lg:px-10 mb-0">
          <ScrollReveal>
            <div className="w-full h-[45vh] md:h-[60vh] relative overflow-hidden">
              <Image
                src={heroImage}
                alt={altTextFor(heroImage, product.name)}
                fill
      ...
      <section className="py-16 md:py-24 border-t border-gray-100">
        <div className="max-w-screen-xl mx-auto px-5 md:px-8 lg:px-10 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          <ScrollReveal>
            ...
            <p className="text-title text-[#1c1c1e] mb-6">{product.name}</p>
      ...
      <ContentBlocks blocks={content?.afterIntro} />
        ...
        <section className="py-14 md:py-20 bg-[#f8f9fa] border-y border-gray-100">
          <div className="max-w-screen-xl mx-auto px-5 md:px-8 lg:px-10">
            <ScrollReveal>
              ...
              <p className="text-title text-[#1c1c1e] mb-10 max-w-xl">
                {chrome.specsHeading}
              </p>
            </ScrollReveal>
      ...
      <ContentBlocks blocks={content?.afterSpecs} />
      ...
      <ContentBlocks blocks={content?.afterFeatures} />
            ...
            {[heroImage, cat.image, product.image].filter(Boolean).map((img, i) => (
              <ScrollReveal key={i} delay={i * 0.1}>
                <div className={`relative overflow-hidden ${i === 0 ? "col-span-2 md:col-span-2 h-64 md:h-80" : "h-36 md:h-40"}`}>
                  <Image
                    src={img!}
                    alt={altTextFor(img, `${product.name} installation ${i + 1}`)}
                    fill
      ...
      <section className="py-14 md:py-20">
        <div className="max-w-screen-xl mx-auto px-5 md:px-8 lg:px-10">
          <ScrollReveal>
            ...
            <p className="text-title text-[#1c1c1e] mb-10 max-w-xl">{chrome.downloadsHeading}</p>
          </ScrollReveal>
      ...
      <ContentBlocks blocks={content?.afterDownloads} />
            ...
            <ScrollReveal>
              <div className="flex items-end justify-between mb-8 md:mb-12">
                <div>
                  ...
                  <p className="text-title text-[#1c1c1e]">
                ...
                <ScrollReveal key={proj.id} delay={i * 0.08}>
                  ...
                  >
                    ...
                    <div className="p-5 md:p-6">
                      ...
                      <p className="font-semibold text-[#1c1c1e] group-hover:text-[#007969] transition-colors mb-1">
                        {proj.name}
                      </p>
              ...
              {related.map((rel, i) => (
                <ScrollReveal key={rel.id} delay={i * 0.08}>
                  ...
                  >
                    {(rel.image ?? cat.image) && (
                      <div className="w-full h-32 relative overflow-hidden mb-5">
                        <Image
                          ...
                          alt={altTextFor(rel.image ?? cat.image, rel.name)}
                          fill
                          className="object-cover group-hover:scale-105 transition-transform duration-700"
                          sizes="33vw"
                        />
                    ...
                    <p className="text-[#1c1c1e] font-semibold mb-2 group-hover:text-[#007969] transition-colors">
                      {rel.name}
                    </p>
              ...
              <ScrollReveal>
                <div className="flex items-end justify-between mb-8 md:mb-12">
                  <div>
                    ...
                    <p className="text-title text-[#1c1c1e]">{chrome.readingHeading}</p>
                  </div>
                  ...
                  <ScrollReveal key={post.slug} delay={i * 0.08}>
                    <Link href={`/technical/blog/${post.slug}`} className="group block bg-white hover:bg-[#f8f9fa] transition-colors h-full">
                      ...
                      <div className="p-5 md:p-6">
                        ...
                        <p className="font-semibold text-[#1c1c1e] leading-snug group-hover:text-[#007969] transition-colors mb-2">{post.title}</p>
      ...
      <ContentBlocks blocks={content?.beforeCta} />
      ...
      <section className="py-20 md:py-32 bg-[#007969]">
        <ScrollReveal>
          <div className="max-w-screen-xl mx-auto px-5 md:px-8 lg:px-10 text-center">
            ...
            <p className="text-title text-white mb-6 max-w-2xl mx-auto">
              {chrome.ctaHeading}
            </p>
