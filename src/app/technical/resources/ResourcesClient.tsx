...
import ResourceRequestForm, { type PendingResource } from "@/components/forms/ResourceRequestForm";
...
export default function ResourcesClient({ resources, hero }: { resources: Resource[]; hero?: PageHero }) {
  ...
  // Requesting now opens a gate that collects name and email so the request can
  // become a CRM contact; the card is only marked "Requested ✓" once the
  // submission actually succeeds.
  function handleRequest(id: string, title: string) {
    setPending({ id, title });
  }
