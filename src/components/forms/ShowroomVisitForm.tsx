...
// Showroom walkthrough shown before the booking form.
const SHOWROOM_INTRO_INSTAGRAM_URL = "https://www.instagram.com/reel/DXbhUpeivBi/";
...
const SHOWROOM_MAPS_URL = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
  `${SHOWROOM_NAME}, ${SHOWROOM_ADDRESS}`
)}`;
const SHOWROOM_WHATSAPP_TEXT = encodeURIComponent(
  `📍 ${SHOWROOM_NAME}\n${SHOWROOM_ADDRESS}\n${SHOWROOM_HOURS}\n${SHOWROOM_MAPS_URL}`
);
...
export default function ShowroomVisitForm({ onClose }: { onClose: () => void }) {
  ...
  // Render the Instagram embed once the intro screen mounts. Instagram's
  // widget script scans the page for `.instagram-media` blockquotes and
  // replaces them with an iframe; if the script is already loaded from a
  // previous mount we just need to re-run its processor.
  useEffect(() => {
    if (!showIntro) return;
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="w-full bg-black flex-shrink-0 flex items-center justify-center py-4">
            <blockquote
              className="instagram-media"
              data-instgrm-permalink={SHOWROOM_INTRO_INSTAGRAM_URL}
              data-instgrm-version="14"
              style={{ margin: 0, width: "100%", maxWidth: 400 }}
            />
          </div>
          <div className="p-6 md:p-8">
            <p className="text-label text-[#007969] mb-3">Take a look inside</p>
            <h2 className="font-heading text-xl font-bold text-[#1c1c1e] mb-3 leading-snug">
              See our Jebel Ali showroom before you book.
            </h2>
            <p className="text-[#6b7280] text-sm leading-relaxed mb-6">
              A quick walkthrough of what&apos;s on display, then we&apos;ll get your visit booked in under two minutes.
            </p>
            <button onClick={() => setShowIntro(false)} className="btn-brand w-full sm:w-auto">
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 flex flex-col items-start justify-center px-8 py-12">
          ...
          <div className="bg-[#f8f9fa] p-5 mb-8 w-full max-w-sm text-sm text-[#3a3a3c] leading-relaxed relative">
            <a
              href={`https://wa.me/?text=${SHOWROOM_WHATSAPP_TEXT}`}
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Send showroom address and map link via WhatsApp"
              title="Send showroom details via WhatsApp"
              className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-[#25D366] text-white hover:bg-[#1ebe57] transition-colors"
            >
              <WhatsAppIcon className="w-4 h-4" />
            </a>
            <p className="font-semibold text-[#1c1c1e] mb-1 pr-8">{SHOWROOM_NAME}</p>
